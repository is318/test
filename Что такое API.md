...
3. Заполнить необходимые параметры запроса (в данном примере не требуется) и отправить запрос.

Для получения списка профессий портала [hh.ru](https://hh.ru/) используется запрос `https://api.hh.ru/professional_roles`.

Пример успешного ответа (статус 200) в нижней части интерфейса:
```json
{
"categories": [
{
